...
def clva_jacobi_generate(input_ids, images_tensor,model, tokenizer, max_new_tokens, max_new_seq_len):
    ...
    past_key_values, first_correct_token = model.jacobi_forward(input_ids=input_ids,images_tensor=images_tensor, max_new_tokens=max_new_tokens, past_key_values=None, use_cache = True, prefill_phase = True)
    ### generation phase
    itr = 0
    eos_reached = False
    while True:
...
def speed_compare(args,CLLMRobotgenerate_implementation):
    ...
    for i in tqdm(data_lst): 
        ...
        ar_end = time.time()
        print(f'ar generated length: {len(output_ids_vla)}')
    ...
    for all_generation_trajectory in per_request_meta_trajectory_records:
        ...
        for n_gram_id in range(len(all_generation_trajectory)):
            ...
            for id, generation_ids in enumerate(generation_trajectory):#第i条数据的迭代轨迹
                # skip initialiation
                if id == 0:
                    continue
                if eos_reached == True:
